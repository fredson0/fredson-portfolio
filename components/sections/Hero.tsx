...
export default function Hero() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative min-h-screen w-full overflow-hidden bg-[#999d9e] text-white"
    >
      <div className="absolute left-0 top-1/2 -translate-y-1/2 px-6 sm:px-10 lg:px-16">
      ...
      <div className="absolute right-0 top-1/2 -translate-y-1/2 px-6 sm:px-10 lg:px-16">
      ...
      <div className="absolute bottom-0 left-0 w-full overflow-hidden pb-6">
        <div
          ref={marqueeRef}
        ...
        >
          <div className="flex shrink-0 items-center">
            {Array.from({ length: marqueeCopies }).map((_, index) => (
              <span
                key={`marquee-a-${index}`}
                className="text-[14vw] font-semibold uppercase leading-none tracking-tight text-white/90 sm:text-[12vw]"
              >
                {marqueeText}
              </span>
            ))}
